...
function Graph(props) {
    ...
    useEffect(() => {
        ...
        if (dbKey) {
            fireStore.collection('users').doc(dbKey).collection('stats').doc(props.statName).onSnapshot((snap) => {
                if (snap.data()) {
                ...
                if (snap.data() && snap.data().data.postData) {
                    ...
                    fireStore.collection('users').doc(dbKey).collection('orgBoard').doc(snap.data().data.postData.postId).onSnapshot((snapshot) => {
...
// the styling is determined by className passed to this component (because it's called from multiple places)
    return <div className={props.from} style={props.from === 'oicGraphs' ? style(statData) : null } > 
        ...
        <div style={{ textAlign: 'center', padding: '15px' }}>
            ...
            {helpBy && helpBy.uid === userInfo.uid && props.from === 'oicGraphs' ? 
                <Button className='btnSecondary' style={{ float: 'right' }} onClick={() => setInputModal(true)}>Add Stat</Button> 
            : null}
        </div>

        <ResponsiveContainer width="100%" aspect={2}>
            <LineChart
                data={props.from === 'statSubmission' ? fullStatData : statData}
                margin={{
